...
export default function Home() {
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    ...
    if (chainId !== 5) {
      ...
      throw new Error("Please switch to the Goerli netowrk!");
    }
  ...
  // and converts the rturned data into a Javascript object with values we can use
  ...
  // Calls the `voteOnProposal` function in the contract, using the passed proposalId and Vote
  ...
  // so when a wallet connects or disconnects, prompts user to connect if not connected
  // and then calls helper functions to fetch the DAO treasury balance,
  // User NFT balance, and number of proposals in the DAO
  useEffect(() => {
    if (!walletConnected) {
